    ...
    public function generalStats()
    {
        ...
        return array(
            ...
            "action_distribution"=>array(
                array("Map Pan", round($p_map_pan,2)), 
                array("Map Zoom in", round($p_map_zoom_in,2)),
                array("Map Zoom out", round($p_map_zoom_out,2)),
                array("Map Click", round($p_map_click,2)),
                array("Click on Fossil", round($p_click_on_fossil,2)),
                array("Enlarge Image", round($p_enlarge_image,2)),
                array("Fossil Selected", round($p_fossil_selected,2)),
                array("Fossil Deselected", round($p_fossil_deselected,2)),
                array("Clear Fossil Selection", round($p_clear_fossil_selection,2)),
                array("Filter Geological Age Change", round($p_filter_geological_change,2)),
                array("Filter Collector Hover", round($p_filter_collector_hover,2)),
                array("Filter Collector Change", round($p_filter_collector_change, 2)),
                array("Filter Genus Hover", round($p_filter_genus_hover, 2)),
                array("Filter Genus Change", round($p_filter_genus_change, 2)),
                array("Feedback Hover", round($p_feedback_hover,2)),
                array("Feedback Click", round($p_feedback_click, 2)),
                array("Upvote", round($p_upvote, 2)),
                array("Click Reply", round($p_click_reply,2)),
                array("Write Comment", round($p_write_comment, 2)),
                array("Submit Comment", round($p_submit_feedback, 2)),
                array("Sharing", round($p_sharing, 2))
            ),
