...
var map_admin = angular.module('map_admin', []).controller('admin_map_feedbacks', function($scope, $http, $compile){
	...
	$scope.showMap = function(feedback_id)
	{
		...
		{
			console.log('/api/map/loadfossils/'+feedback['genus']+'/-1/'+feedback['age_max']+'/'+feedback['age_min']+'/'+feedback['collector']+'/-1/-1/-1/-1/-1');
			$http.get('/api/map/loadfossils/'+feedback['genus']+'/-1/'+feedback['age_max']+'/'+feedback['age_min']+'/'+feedback['collector']+'/-1/-1/-1/-1/-1').success(function(data, status, headers, config){
				data.forEach(function(item, index){
					var info = [];
					info['lat'] = item['lat'];
					info['lng'] = item['lng'];
